...
INPUT=1; %1 白高斯\ %2==>AR1 3==>AR2 4==>AR4
IN_MODE=1; %1弱 2中 3強
N_L=7;
for k = 1:ensamble
    % White Noise
    ...
    %     pb = 0.01;
    %     SIRdB = -20;
    %     p = binornd(1,pb,1,N); % Bernoulli Process
    %     var_b = var_d/(10^(SIRdB/10));
    %     b = sqrt(var_b)*randn(1,N); % Gaussian Process
    %     imp = p.*b;
    %     dn2 = dn1 + imp;
    ...
    % [e_m,w_m_hat,update_ratio_m(k,1),detection_m(k,1),false_alarm_m(k,1)] = DS_LMS_mix1(x,dn2,w_m_hat,P_up,var_v,imp);
    
    w1_hat=[w1_hat;zeros(length(w)-N_L,N)];
    w2_hat=[w2_hat;zeros(length(w)-N_L,N)];    
    w_d_hat=[w_d_hat;zeros(length(w)-N_L,N)];
    w_e_hat=[w_e_hat;zeros(length(w)-N_L,N)];
...
%NMSD_m = sum(NMSD_m,1)/ensamble;

% NMSD1 = smoothing(NMSD1);
% NMSD2 = smoothing(NMSD2);
% NMSD_d = smoothing(NMSD_d);
% NMSD_e = smoothing(NMSD_e);
% NMSD_m = smoothing(NMSD_m);

NMSD1 = 10*log10(NMSD1);
NMSD2 = 10*log10(NMSD2);
NMSD_d = 10*log10(NMSD_d);
NMSD_e = 10*log10(NMSD_e);
%NMSD_m = 10*log10(NMSD_m);
